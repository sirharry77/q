...
function selectAyat(ayatData) {
    ...
    // Update the count of occurrences of "GOD" for the current verse
    const godCountInVerse = countGodOccurrencesInTranslation(ayatData.translation);

    // Calculate the count of occurrences of "GOD" up to the current verse
    let cumulativeGodCount = 0;
    for (let i = 0; i < ayatData.surahNumber - 1; i++) {
        const surah = surahData[i + 1];
        for (let j = 0; j < surah.ayat.length; j++) {
            cumulativeGodCount += countGodOccurrencesInTranslation(surah.ayat[j].translation);
        }
    }
    cumulativeGodCount += godCountInVerse;

    // Update the total count
    totalGodCount = cumulativeGodCount;
...
// Assuming you have already loaded the translation text into the '.translation' element

// Get the translation text
//const translationText = document.querySelector(".translation").textContent;

// Count the occurrences of the word "God" (case-sensitive)
//const godCount = (translationText.match(/\bGod\b/g) || []).length;

// Display the count on the HTML page
//document.getElementById("godCount").textContent = godCount;	
...
// Initialize total count of occurrences of "GOD"
let totalGodCount = 0;

// Function to update the count and display it
function updateGodCount() {
    // Display the total count on the HTML page
    document.getElementById("godCount").textContent = totalGodCount;
}

// Function to count occurrences of "GOD" in the translation
function countGodOccurrencesInTranslation(translationText) {
    return (translationText.match(/\bGOD\b/g) || []).length;
}

// Function to show a specific verse and update the count
function showSurahVerse(surahNumber, ayatNumber) {
    ...
    if (surah && surah.ayat && surah.ayat.length > 0) {
        ...
        // Retrieve the translation text up to the current verse
        const translationText = surah.ayat.slice(0, currentAyatIndex + 1).map(verse => verse.translation).join(' ');

        // Count occurrences of "GOD" in the translation up to the current verse
        const godCountInTranslation = countGodOccurrencesInTranslation(translationText);

        // Update the total count
        totalGodCount = godCountInTranslation;
